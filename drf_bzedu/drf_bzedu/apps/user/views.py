...
from rest_framework import status as ccokt, status
from rest_framework.viewsets import ViewSet
from drf_bzedu.libs.geetest import GeetestLib
from drf_bzedu.settings import count
from user.models import MyUser
from user.serializer import RegisterModelSerializer, SmsLoginSerializer
...
class SmsLogin(ViewSet):
    def sms_login(self,request,*args,**kwargs):
        ...
        if not user:
            return Response({'msg': '用户不存在'})
        data = SmsLoginSerializer(user).data
...
#短信登录验证码逻辑
class SendLogin(APIView):
    def get(self, request, mobile):
        # 第一步连接数据库
        redis_connection = get_redis_connection("sms_login")
        # 1.判断手机验证码是否在60s内发送过短信
        mobile_code = redis_connection.get("sms_%s" % mobile)
        if mobile_code is not None:
            return Response({"message": "您已经发送过短信了"}, status=ccokt.HTTP_400_BAD_REQUEST)
...
class SmsLoginCheckPhoneId(APIView):
    def get(self, request, phone_id):
        # request.query_params.get()
        print(phone_id)
        # 判断手机号是否合法
        ...
        print(user)
        if user is None:
            return Response({"message": "请先注册"}, status=ccokt.HTTP_400_BAD_REQUEST)
        return Response({"message": "ok"})